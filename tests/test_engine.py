...
def test_condition_must_match_full_dropdown_text_exactly():
    """Asset | GPE Condition must be selected from the Excel dropdown built
    from the Condition | Disposal Reason sheet, not typed in by hand. A real
    dropdown selection reproduces the option's full text exactly, so the
    valid value is the row's full text (e.g.
    "Good (No visible damage, no repairs completed)"), matched with exact
    capitalization -- not the short word before the bracket, and not a
    case-insensitive match."""
    valid = list(CONDITIONS)     # the exact, full dropdown text
    # A stray leading/trailing space is not a meaningful difference (nobody
    # should be told to "fix" invisible whitespace), so these stay valid too.
    valid_rows = valid + [CONDITIONS[1] + " ", " " + CONDITIONS[1]]
    invalid = ["Good", "GOOD", "good", "NEW", "poor",   # short forms / wrong case
              CONDITIONS[1].upper(), CONDITIONS[1].lower(),
              "Very good"]

    rows = [GOOD_ROW]
    for index, value in enumerate(valid_rows):
    ...
    expect("the full dropdown text, exactly as the reference sheet writes it, "
    ...
    for index, value in enumerate(invalid):
        ...
        expect(f"{value!r} is not the exact dropdown text and is reported invalid",
               issue is not None and value.strip() in issue.values,
               str(result_bad.issues))

    # The feedback must list the exact dropdown options, derived from the
    # sheet at run time (not a hard-coded string), in the sheet's own order.
    ...
    expect("the exact dropdown options are listed for the team",
           issue.note_items == valid, str(issue.note_items))
    expect("dropdown selection is mentioned so the team knows not to type it in",
           "dropdown" in issue.note.lower(), issue.note)

    document = build_feedback(result_bad)
    text = render_text(document)
    expect("the feedback tells the team to select from the dropdown, not type it in",
           ("dropdown" in text.lower()) and ("not type" in text.lower()), text)
    expect("the exact dropdown options are printed in the feedback for the team to pick from",
           all(option in text for option in valid), text)
